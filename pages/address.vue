    ...
    <MainLayout>
        <div id="AddressPage" class="mt-4 max-w-[500px] mx-auto px-2">
            <div class="mx-auto bg-white rounded-lg p-3">
                <div class="text-xl text-bold mb-2">ที่อยู่ในการจัดส่ง</div>
                <form @submit.prevent="submit()">
                    <TextInput class="w-full" placeholder="ชื่อ-สกุล" v-model:input="contactName" inputType="text"
                        :error="error && error.type == 'contactName' ? error.message : ''" />

                    <TextInput class="w-full mt-2" placeholder="ที่อยู่" v-model:input="address" inputType="text"
                        :error="error && error.type == 'address' ? error.message : ''" />



                    <TextInput class="w-full mt-2" placeholder="อำเภอ" v-model:input="city" inputType="text"
                        :error="error && error.type == 'city' ? error.message : ''" />

                    <TextInput class="w-full mt-2" placeholder="จังหวัด" v-model:input="country" inputType="text"
                        :error="error && error.type == 'country' ? error.message : ''" />

                    <TextInput class="w-full mt-2" placeholder="รหัสไปรษณีย์" v-model:input="zipCode" inputType="text"
                    ...
                    <button :disabled="isWorking" type="submit" class="
                                ...
                                rounded-full
                            ">
                        <div v-if="!isWorking">อัพเดตที่อยู่</div>
